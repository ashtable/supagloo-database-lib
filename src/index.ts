...
// `scriptureBook` column and any future consumer normalize identically. The recognized
// book set is this normalizer's match vocabulary, NOT a canon claim — which books a
// translation contains is YouVersion's authority, not ours. Explicitly named (not
// `export *`) like `github.ts`/`s3-keys.ts`.
export {
  SCRIPTURE_BOOKS,
  deriveScriptureBook,
  isScriptureBookCode,
...
// The ONE rule reconciling a scene's authored length with its narration's MEASURED
// length, so a verse is never cut off mid-sentence. Shared because the render
// (templates.ts) and the studio (timeline + preview) must agree on how long a scene is.
export { effectiveSceneDurationSeconds } from "./manifest-duration";
